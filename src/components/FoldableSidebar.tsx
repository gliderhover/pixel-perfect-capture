    ...
    >
      <div className="flex items-center justify-between gap-1 border-b border-border/20 px-1.5 py-2 shrink-0">
        ...
        <button
          ...
          className="ml-auto flex h-9 w-9 shrink-0 items-center justify-center rounded-xl text-foreground/90 transition-colors hover:bg-primary/10"
        ...
        {items.map(({ id, label, icon: Icon }) => {
          ...
          return (
            <button
              key={id}
              type="button"
              onClick={() => (isRules ? onOpenRules() : onNavigate(id as AppTab))}
              className={cn(
                "flex items-center gap-3 rounded-xl py-2.5 text-left transition-all",
